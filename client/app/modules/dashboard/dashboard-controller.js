    ...
    function renderTasksAndEventsToCalendar(isUpdate) {
        ...
        _.each($scope.myTasks, function(task) {
            if (task.element && task.element.type === "task-project") {
                ...
                $scope.events.push({type: "task", _id: task._id, title: title, project: (task.project._id) ? task.project._id : task.project, start: dateStart, end: dateEnd, "backgroundColor": (task.__v > 0) ? "#2196F3" : "#FFC107", allDay: false});
            }
        });
