...
EventBus.onSeries('Project.Inserted', function(request, next){
    ...
    //         BuilderPackage.findOne({project: request._id}, function(err, builderPackage){
    //             if (err) {return next();}
    //             else {
    ...
    //                     project: request,
    //                     registryLink: config.baseUrl + 'signup-invite?packageInviteToken=' + result.builderPackage._id,
    ...
    //     }
    // }
    if(request.type === 'FromBuilderToHomeOwner') {
        if (!request.user._id) {
